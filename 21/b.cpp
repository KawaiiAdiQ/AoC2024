...
using MemoKey = tuple<string, int, bool>;
map<MemoKey, long long> memo;

long long findShortestSequence(string line, int depth, bool isArrowPad = false){
    MemoKey memokey = make_tuple(line, depth, isArrowPad);
...
int main(){
    ...
    while(getline(cin, line)){
        long long p = findShortestSequence(line, 25);
